...
(defmacro defexpandtable (name &rest clause)
  ...
  ;; body
  `(setf (gethash ',name *expandtables*) (make-expandtable ',clause)))

(defun pprint-defexpandtable (stream exp)
  (setf stream (or stream *standard-output*))
  (funcall (formatter "~:<~W~^ ~1I~@_~W~^~@{ ~_~W~}~:>") stream exp))
      ...
      (flet ((add-expander (key value)
               (if (null (gethash key ht))
                   ...
                   (restart-case (error 'expander-conflict :name key)
                     (use-prev ()
                         :report "Use previous expander, discard new one.")
                     (use-next ()
                         :report "Use new expander, discard old one."
                       (setf (gethash key ht) value))))))
        (dolist (clause clauses ht)
          (ecase (car clause)
            (:use
             (dolist (elt (cdr clause))
               (maphash #'add-expander (find-expandtable elt))))
            (:add
             (loop :with value = (cadr clause)
                   :for key :in (cddr clause)
                   :do (add-expander key value)))))))))

;;;; Standard expandtable.

(defun |quote-expander| (whole env) (declare (ignore env)) whole)
...
(defun |let-expander| (whole env)
  (destructuring-bind
    ...
    (multiple-value-bind (body decls)
        (alexandria:parse-body body)
      `(,op
        ,(loop :for elt :in binds
               :if (symbolp elt)
                 :collect elt
               :else
                 :collect `(,(car elt) ,(expand (cadr elt) env)))
        ,@decls
        ,@(expand* body
                   (augment-environment env
                                        :variable (mapcar
                                                    ...
                                                    binds)
                                        :declare (alexandria:mappend #'cdr
                                                                     decls)))))))
...
(defun expand* (sub-forms env)
  (mapcar (lambda (sub-form) (expand sub-form env)) sub-forms))
...
(defun |flet-expander| (whole env)
  (destructuring-bind
    ...
    (multiple-value-bind (body decls)
        (alexandria:parse-body body)
      `(,op
        ,(loop :for (name params . body) :in binds
               :collect `(,name ,(expand-params params env)
                          ,@(loop :for elt :in body
                                  :collect (expand elt env))))
        ,@decls
        ,@(expand* body
                   (augment-environment env :function (mapcar #'car binds)))))))
...
(defun |labels-expander| (whole env)
  (destructuring-bind
    ...
    (multiple-value-bind (body decls)
      ...
      `(,op
        ,(loop :for (name params . body) :in binds
               :do (setf env (augment-environment env :function (list name)))
               :collect `(,name ,(expand-params params env)
                          ,@(expand* body env)))
        ,@decls
        ,@(expand* body
...
(defun expand-params (params env)
  (loop :for param :in params
        :if (symbolp param)
          :collect param
        :else
          :collect `(,(car param) ,(expand (cadr param) env)
                     ,@(when (caddr param)
                         `(,(caddr param))))))
...
(defun |lambda-expander| (whole env)
  (destructuring-bind
    ...
    (multiple-value-bind (body decls)
        (alexandria:parse-body body)
      `#'(,op ,(expand-params params env) ,@decls
          ,@(expand* body
                     (augment-environment env
...
(defun |unwind-protect-expander| (whole env)
  (destructuring-bind
      (op form . cleans)
      whole
    `(,op ,(expand form env) ,@(expand* cleans env))))

(defun |throw-expander| (whole env)
  (destructuring-bind
      (op tag result)
      whole
    `(,op ,(expand tag env) ,(expand result env))))
...
(defun |setq-expander| (whole env)
  (destructuring-bind
      ...
      whole
    `(,op
      ,@(loop :for (place form) :on forms :by #'cddr
              :collect place
...
(defun |if-expander| (whole env)
  (destructuring-bind
      (op pred then . else)
      whole
    `(,op ,(expand pred env) ,(expand then env)
      ,@(when else
          `(,(expand (car else) env))))))

(defun |locally-expander| (whole env)
  (destructuring-bind (op . body) whole `(,op ,@(expand* body env))))

(defun |eval-when-expander| (whole env)
  (destructuring-bind (op cond . body) whole `(,op ,cond ,@(expand* body env))))
...
(defmacro trace-expanders (name)
  (let ((table (find-expandtable name)) names)
    (maphash
...
(defmacro with-trace-out ((expandtable filename) form)
  `(unwind-protect
       (with-open-file (*trace-output* ,filename :direction :output
  ...
  (let ((result (macroexpand form env)))
    (if (atom result) ; may be expanded into atom directly.
        ...
        (if (typep (car result) '(cons (eql lambda) *))
            ...
            (funcall (get-expander (car result) cont) result env)))))
...
(defun expand-sub-form (form env) `(,(car form) ,@(expand* (cdr form) env)))
...
(defun remove-the (form)
  (if (and (listp form) (eq 'the (car form)))
...
(defun parse-bubble-let (let)
  (destructuring-bind
    ...
    (multiple-value-bind (body decls)
        (alexandria:parse-body body)
      (multiple-value-bind (symbol-macrolet-cadr new-binds)
          (parse-bubble-binds binds)
        (multiple-value-call #'values
          new-binds
          (loop :for declare-form :in decls
                :collect (cons 'declare
                               (loop :for option :in (cdr declare-form)
                                     :with alist
                                           = (mapcar
...
(defun parse-bubble-binds (binds)
  (labels ((rec (binds &optional sm-cadr new-binds)
           ...
           (body (bind rest sm-cadr new-binds)
             (if (symbolp bind)
                 (let ((gensym
                        (gensym (remove-if #'digit-char-p (symbol-name bind)))))
                   (rec rest (cons `(,bind ,gensym) sm-cadr)
                        (cons gensym new-binds)))
                 (let ((gensym
                        (gensym
                          (remove-if #'digit-char-p (symbol-name (car bind))))))
  ...
  (destructuring-bind
    ...
    (let ((expanded
      ...
      (cond ((null expanded) nil)
            ((null (cdr expanded)) (car expanded))
            (t
             (multiple-value-bind (let-form args)
                 (bubble-up expanded env)
               (if let-form
...
(defun flatten-nested-op (op args-list &key (args #'cdr))
  (labels ((rec (list &optional acc)
           ...
           (body (form rest acc)
             (if (and (listp form) (eq op (car form)))
...
(defun pure-fun-form-p (form env)
  (or (constantp form env)
      ...
      (typep form '(cons (eql function) (cons (cons (eql lambda) *) *)))))
    ...
    (let* ((expanded
            (remove-if
              (lambda (x)
                (and (constantp x env)
                     (alexandria:emptyp
                       (introspect-environment:constant-form-value x env))))
              (flatten-nested-op 'concatenate (expand* rest env)
                                 :args #'cddr))))
      (cond
       ((null expanded)
        (if (constantp type env)
            ...
            `(coerce nil ,type)))
       ((null (cdr expanded)) `(coerce ,(car expanded) ,type))
       (t
        (if (not (constantp type env)) ; to keep eval order.
            `(,op ,type ,@expanded)
            (multiple-value-bind (let-form args)
                (bubble-up expanded env)
              (if let-form
...
(defun |*-expander| (form env)
  (let ((expanded
         (remove-if
           ...
           (flatten-nested-op '*
                              (let ((*expandtable*
                                     (find-expandtable 'standard)))
                                (expand* (cdr form) env))))))
    (multiple-value-bind (zerop args)
        (sieve-zero expanded env)
      (if zerop
          (if args
              `(progn ,@args 0)
              0)
...
(defun |optimized-let-expander| (form env)
  (destructuring-bind
    ...
    (multiple-value-bind (body decls)
      ...
      (cond ;; Remove-null-bind
            ((and (null binds) (null decls) (null (cdr body))) (car body))
            ;; Expand-useless-bind
...
(defun |optimized-locally-expander| (form env)
  (destructuring-bind
      ...
      form
    (multiple-value-bind (body decls)
        (alexandria:parse-body (expand* body env))
  ...
  (destructuring-bind
    ...
    (cond ((null body) nil)
          ...
          (t
           (let* ((pos
                   (position-if
                     (lambda (x) (typep x '(cons (eql return-from) *))) body))
                  ;; If body has return-from form, we can ignore rest forms.
                  (body
                   (if pos
                       (subseq body 0 (1+ pos))
                       body)))
             (multiple-value-bind (let-form forms)
                 (bubble-up body env)
               (if let-form
...
(defun |optimized-return-from-expander| (form env)
  (destructuring-bind
    ...
    (if (and (listp body) (eq 'return-from (car body)))
        body
        (multiple-value-bind (let-form main)
            (bubble-up (list body) env)
          (if let-form
...
(defun |format-expander| (form env)
  (destructuring-bind
      (op target string . args)
      form
    (if (constantp string env)
        `(,op ,(expand target env)
          ,(expand
             `(formatter
               ,(introspect-environment:constant-form-value string env))
             env)
          ,@(expand* args env))
        (expand-sub-form form env))))
...
(defun |optimized-the-expander| (form env)
  (destructuring-bind
      ...
      form
    (setf body (expand body env))
    (if (and (listp body) (eq 'the (car body)) (equal type (cadr body)))
        body
        `(,op ,type ,body))))
